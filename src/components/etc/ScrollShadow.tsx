...
export function ScrollShadow(props: { children: React.ReactNode; className?: string; scrollableClassName?: string }) {
  ...
  useEffect(() => {
    ...
    function handleScroll() {
      if (!content || !shadowTop || !shadowBottom || !shadowLeft || !shadowRight) return;
  ...
  return (
    <div className={cn(props.className, "relative overflow-hidden z-base")} ref={wrapperEl}>
      <div
      ...
      ></div>
      <div className={cn("hide-scrollbar overflow-auto", props.scrollableClassName)} ref={scrollableEl}>
        {props.children}
      </div>
    </div>
  );
